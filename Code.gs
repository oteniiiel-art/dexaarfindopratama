...
const DEFAULT_SPREADSHEET_ID = 'ISI_SPREADSHEET_ID_DI_SINI';
const DEFAULT_DRIVE_FOLDER_ID = 'ISI_FOLDER_ID_DI_SINI'; // untuk simpan lampiran foto/nota

const SHEET_USERS = 'Users';
const SHEET_PENGADAAN = 'Pengadaan';

const USER_HEADERS = ['id','nama','username','password','jabatan','divisi','role','email','hp'];
const PENGADAAN_HEADERS = ['id','kode','namaPerangkat','instansi','kategori','merk','jumlah','satuan','hargaSatuan','total','vendor','noPO','divisi','tglAjukan','tglButuh','status','catatan','lampiranUrl','createdAt'];
...
function getSpreadsheet(idFromClient) {
  const id = idFromClient || DEFAULT_SPREADSHEET_ID;
...
function setupInitialData() {
  const ss = getSpreadsheet(DEFAULT_SPREADSHEET_ID);
  const users = getOrCreateSheet(ss, SHEET_USERS, USER_HEADERS);
  if (users.getLastRow() < 2) {
    users.appendRow(['U001', 'Admin K24 IT', 'admin123', 'admin123', 'IT Manager', 'IT', 'Admin IT', '', '']);
  }
